...
const SYSTEM_PROMPT = `You are Kipcoachee — an elite AI running coach built into Cade.
...
serve(async (req) => {
  ...
  const [planA, planB] = await Promise.all([
    admin.from("training_plan").select("id").eq("user_id", invite.from_user).eq("is_active", true).order("created_at", { ascending: false }).limit(1).maybeSingle(),
    admin.from("training_plan").select("id").eq("user_id", invite.to_user).eq("is_active", true).order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
  ...
  const userPrompt = `Create a ${invite.invite_type} workout session for ${dateStr}.
...
Mode: ${invite.invite_type === "parallel" ? "PARALLEL — they do their own reps but share warm-up, cool-down, and match rest periods where possible." : "COMBINED — merge into one shared session that respects both plans."}`;
