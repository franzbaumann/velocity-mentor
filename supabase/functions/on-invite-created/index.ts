...
serve(async (req) => {
  ...
  if (webhookSecret) {
    ...
    if (providedSecret !== webhookSecret) {
      console.warn("[on-invite-created] Invalid or missing webhook secret");
  ...
  const body = (await req.json().catch(() => ({}))) as {
    type?: string;
    table?: string;
    record?: { id?: string; session_id?: string | null };
  };
  ...
  const { data: dbInvite } = await admin
    ...
    .maybeSingle();
  if (!dbInvite) {
    console.error("[on-invite-created] Invite not found", inviteId);
  ...
  if (!fromUser || !toUser || !dateStr) {
    console.error("[on-invite-created] Missing required fields", { inviteId });
