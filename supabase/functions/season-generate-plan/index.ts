...
function enforceMinimumQualitySessions(
  ...
  for (const week of weeks) {
    ...
    if (min === 0 && phil.includes("lydiard") && week.phase === "base") {
      for (const w of week.workouts) {
        if (QUALITY_TYPES.has(w.type)) {
          w.type = "easy";
          w.session_library_id = w.session_library_id?.startsWith("a-") ? w.session_library_id : "a-01";
        }
      }
      continue;
    }
    ...
    if (phil.includes("norwegian")) {
      ...
      let added = 0;
      for (const targetDay of doubleThresholdDays) {
        if (added >= deficit) break;
        const existing = week.workouts.filter((w) => w.day_of_week === targetDay);
        const hasQuality = existing.some((w) => QUALITY_TYPES.has(w.type));
        if (hasQuality) continue;
        // Convert the easy session on this day to a threshold session (AM), or add if none exists
      ...
      // If still short, fall through to generic logic below
    ...
    const remaining = min - week.workouts.filter((w) => QUALITY_TYPES.has(w.type)).length;
