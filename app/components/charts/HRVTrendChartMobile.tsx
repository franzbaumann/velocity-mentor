...
export const HRVTrendChartMobile: FC<Props> = ({ data }) => {
  ...
  const hrvValues = data.map((d) => d.CTL);
  ...
  const [topLabel, bottomLabel] = useMemo(() => {
    if (!hrvValues.length) return ["", ""];
    const min = Math.min(...hrvValues);
    const max = Math.max(...hrvValues);
    return [`${Math.round(max)} ms`, `${Math.round(min)} ms`];
  }, [hrvValues]);
    ...
    <View>
        ...
        >
          ...
          {activePoint && activeDatum ? (
            <View
              pointerEvents="none"
              style={[styles.tooltip, { borderColor: theme.cardBorder, backgroundColor: theme.cardBackground }]}
            >
              <Text style={[styles.tooltipLabel, { color: theme.textMuted }]}>{format(new Date(activeDatum.date), "MMM d")}</Text>
              <Text style={[styles.tooltipValue, { color: theme.textPrimary }]}>{`${Math.round(activeDatum.CTL)} ms`}</Text>
            </View>
          ) : null}
        </View>
      </View>
