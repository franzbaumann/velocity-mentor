...
type Point = { date: string; vo2max: number };
...
export const VO2maxTrendChartMobile: FC<Props> = ({ data }) => {
  ...
  const [topLabel, bottomLabel] = useMemo(() => {
    ...
    const min = Math.min(...values);
    const max = Math.max(...values);
    return [`${max.toFixed(1)} ml/kg/min`, `${min.toFixed(1)} ml/kg/min`];
  }, [values]);
          ...
          {activePoint && activeDatum ? (
            ...
            >
              ...
              <Text style={[styles.tooltipValue, { color: theme.textPrimary }]}>
                {activeDatum.vo2max.toFixed(1)} ml/kg/min
              </Text>
            </View>
          ) : null}
        </View>
