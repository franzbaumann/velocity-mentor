...
type Props = {
  data: Point[];
  scale: string[]; // length 5, index 0-4
  color: string;
};
...
export const WellnessCheckChartMobile: FC<Props> = ({ data, scale, color }) => {
  ...
  const [topLabel, bottomLabel] = useMemo(() => {
    const top = scale[4] || scale[3] || "";
    const bottom = scale[0] || scale[1] || "";
    return [top, bottom];
  }, [scale]);
  ...
  const activeLabel =
    activeDatum != null ? scale[Math.round(Math.max(0, Math.min(4, activeDatum.value)))] ?? `${activeDatum.value}` : null;
    ...
    <View>
      <View style={styles.row}>
        ...
        >
          ...
          {activePoint && activeDatum && activeLabel != null ? (
            <View
              pointerEvents="none"
              style={[
                styles.tooltip,
            ...
            >
              ...
              <Text style={[styles.tooltipValue, { color: theme.textPrimary }]}>{activeLabel}</Text>
            </View>
          ) : null}
        </View>
      </View>
