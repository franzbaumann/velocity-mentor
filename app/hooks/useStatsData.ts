...
const now = new Date();
const oldest6w = fmt(subWeeks(now, 6));
...
function buildFitnessSeries(activities: ActivityRow[], readinessRows: ReadinessRow[]): FitnessPoint[] {
  ...
  const fromReadiness = readinessRows
    .filter((r) => {
      ...
      return (ctl != null || atl != null || tsb != null) && r.date >= oldest6w && r.date <= fmt(now);
    })
  ...
  return computeFitnessCurves(statsActs, oldest6w, fmt(now));
}
